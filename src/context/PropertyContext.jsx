...
import { deleteProperty, editProperty, getProperty, getPropertys, postProperty } from "../service/property";
...
export const PropertyProvider = ({ children }) => {
    ...
    const _postProperty = async (data) =>{
        try {
    ...
    const _deleteProperty = async (id) =>{
        try {
            ...
            if(delProperty.status === 200) setPropertysAll(propertysAll.filter((property) => property._id !== id))
        } catch (error) {
    ...
    return (
        <PropertyContext.Provider value={{ _getProperty, _getPropertys, propertysAll, _postProperty, _putProperty, _deleteProperty }}>
            {children}
